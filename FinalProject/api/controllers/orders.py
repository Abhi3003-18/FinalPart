...
def make_payment(db: Session, order_id: int, payment_amount: float):
    try:
        # Retrieve the order
        order = db.query(model.Order).filter(model.Order.id == order_id).first()
        if not order:
        ...
        if payment_amount < order.total_price:
            raise HTTPException(
                ...
                detail=f"Insufficient payment. Amount due: {order.total_price - payment_amount:.2f}"
            )

        # Mark the order as paid
        order.is_paid = True
        order.payment_received = payment_amount  # Optionally record the payment amount
        db.commit()
        db.refresh(order)
    ...
    return {"message": "Payment successful", "order_id": order_id, "paid_amount": payment_amount}